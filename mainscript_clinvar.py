...
# example run:
# python CommonUniqueVariants.py -v vcf1.vcf -v vcf2.vcf -v vcf3.vcf -v vcf4.vcf -c cosmic.vcf -ac dbSNP.vcf -cl clinvar.vcf
...
def non_common_mutations(vcf_positions, vcf_variants, commonMutationsVar,commonMutationPos):
	...
	for sample1 in samples:
		...
		fout.close()
		print(len(non_common_var[sample1]))
...
def common_mutations(non_common_var, non_common_pos, cosmicMutations,clinvarMutations):
	...
	for sample1 in samples:
		...
		clinvarOverlap[sample1] = (non_common_var[sample1].intersection(clinvarMutations))
...
def main():
    ...
    clinvar = parseClinvar(args.clinvar)
